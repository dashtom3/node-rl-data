...
class Test {
    ...
    async doTest() {
        ...
        //     },2000)
        //    }
        const workSheetsFromFile = xlsx.parse(path.join(__dirname, 'mac.xlsx'));
        const workdata = xlsx.parse(path.join(__dirname, '数据10-23.xlsx'));
        var analyseMac, data1;
        workdata.forEach(item => {
            item.data.forEach(single => {
                analyseMac = (single[0].split(':')[0] + single[0].split(':')[1] + single[0].split(':')[2]).toUpperCase()
                workSheetsFromFile.forEach(item1 => {
                    item1.data.forEach(single1 => {
                        if (single1[0]) {
                            if (single1[0].indexOf(analyseMac) > -1) {
                                single.push(single1[1])
                            }
                        }
                    })
                })
        ...
        var buffer = xlsx.build(workdata);
        fs.writeFileSync('resultest.xlsx', buffer);
    }
